...
[status.getValuesBulk] = invoke(psospaDeviceObj, 'psospaGetValuesBulk', startIndex,...
    pSamplesCollected, firstSegmentIndex, toSegmentIndex, downSampleRatio, downSampleRatioMode, pOverflow);
...
for k = segmentCount+1:-1:1 
    triggerInfo(k) = psospaStructs.tPicoTriggerInfo.members;
    ...
    triggerInfo(k).missedTriggers=0;
    triggerInfo(k).timeStampCounter=0;
end

pTriggerInfo = libpointer('tPicoTriggerInfoPtr',triggerInfo);

[status.getTriggerInfo] = invoke(psospaDeviceObj,'psospaGetTriggerInfo',triggerInfo, firstSegmentIndex, segmentCount);
